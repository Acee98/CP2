...
if ($user_id <= 0) {
    $user_id = mailbox_resolve_user_id($conn, $_SESSION['email'], 'user');
}
...
if ($user_id > 0) {
    $ticketStmt = $conn->prepare(
        'SELECT id, subject, description, status FROM tickets WHERE user_id = ? ORDER BY created_at DESC'
    );
    if ($ticketStmt !== false) {
        $ticketStmt->bind_param('i', $user_id);
        $ticketStmt->execute();
        $ticketStmt->bind_result($t_id, $t_subject, $t_description, $t_status);
        while ($ticketStmt->fetch()) {
            $user_tickets[] = [
                'id'          => (int) $t_id,
                'subject'     => $t_subject,
...
$allowed_nav_tabs = ['tickets', 'messages', 'profile', 'settings'];
$active_nav_tab = $_GET['tab'] ?? 'tickets';
if (!in_array($active_nav_tab, $allowed_nav_tabs, true)) {
    $active_nav_tab = 'tickets';
}
...
$mailbox_role = 'user';
$mailbox_current_user_id = $user_id;
$mailbox_conversations = $user_id > 0 ? mailbox_fetch_conversations($conn, 'user', $user_id) : [];
$mailbox_active_ticket_id = isset($_GET['ticket_id']) ? (int) $_GET['ticket_id'] : 0;
$mailbox_messages = [];
$mailbox_active_ticket = null;

if ($mailbox_active_ticket_id > 0 && mailbox_can_access_ticket($conn, $mailbox_active_ticket_id, 'user', $user_id)) {
        ...
        <section class="showcase">
            ...
            <div class="page-content" id="page-tickets">
                ...
                <?php if ($ticket_success): ?>
                    <div class="utilities-notice"><?= htmlspecialchars($ticket_success) ?></div>
                <?php endif; ?>
                <?php if ($ticket_error): ?>
                    <div class="utilities-notice-error"><?= htmlspecialchars($ticket_error) ?></div>
                <?php endif; ?>

                <!-- Toolbar: filter tabs left, New Ticket button right -->
                <div class="tickets-toolbar">
                    <div class="tickets-filter-tabs" id="user-tickets-filter-tabs">
                    ...
                    <div class="tickets-list-body" id="user-tickets-body">
                        ...
                        <?php else: ?>
                            <?php foreach ($user_tickets as $ticket): ?>
                                <?php
                                    $statusClass = preg_replace('/[^a-z]/', '', strtolower($ticket['status']));
                                    $statusLabel = ucfirst($ticket['status']);
                                    $descPreview = mb_strlen($ticket['description']) > 80
                                        ? mb_substr($ticket['description'], 0, 80) . '…'
                                        : $ticket['description'];
                                    $searchBlob = strtolower('#' . $ticket['id'] . ' ' . $ticket['subject'] . ' ' . $ticket['description'] . ' ' . $ticket['status']);
                                ?>
                                <div class="ticket-row" data-status="<?= htmlspecialchars($ticket['status']) ?>"
                                    data-search="<?= htmlspecialchars($searchBlob) ?>">
                                    <span class="tickets-col-id">#<?= str_pad((string) $ticket['id'], 4, '0', STR_PAD_LEFT) ?></span>
                                    <span class="tickets-col-subject"><?= htmlspecialchars($ticket['subject']) ?></span>
                                    <span class="tickets-col-description"><?= htmlspecialchars($descPreview) ?></span>
