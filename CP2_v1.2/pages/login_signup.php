...
// Already logged in — send to the correct dashboard instead of showing login.
if (isset($_SESSION['email'], $_SESSION['role'])) {
    $loggedInRole = $_SESSION['role'];
    if ($loggedInRole === 'admin') {
        header('Location: admin.php');
    } elseif ($loggedInRole === 'techn') {
...
function showSuccess($message) {
    return !empty($message) ? "<div style='padding:12px; background-color: #b8d4be; border-radius:6px; text-align:center; margin-bottom:20px; color:#1e5c2e; font-weight:600; font-size: 14px; width:100%;'>" . htmlspecialchars($message, ENT_QUOTES, 'UTF-8') . "</div>" : '';
}
